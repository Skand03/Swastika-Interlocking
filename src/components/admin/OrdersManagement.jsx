...
export default function OrdersManagement({ language, orders, handleStatusChange }) {
  ...
  return (
    <>
      <section className="space-y-8 print:hidden" id="orders-management">
        {/* Header */}
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-grow md:flex-grow-0">
              <input
                ...
                className="bg-white border border-gray-200 rounded-xl pl-9 pr-4 py-2 w-full md:w-64 focus:ring-2 focus:ring-[#E8650A] outline-none text-sm shadow-sm"
            ...
            <select
              ...
              className="bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none shadow-sm cursor-pointer"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            { label: isHindi ? 'प्रगति पर' : 'Processing',  count: processingOrders, color: '#1565C0', icon: 'sync'           },
            { label: isHindi ? 'भेजे गए' : 'Dispatched',   count: shippedOrders,    color: '#7B1FA2', icon: 'local_shipping' },
            { label: isHindi ? 'डिलीवर' : 'Delivered',     count: deliveredOrders,  color: '#2E7D32', icon: 'check_circle'   },
          ].map(s => (
            <div key={s.label} className="bg-white rounded-xl p-5 shadow-sm border-l-4" style={{ borderColor: s.color }}>
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-gray-400 text-xs font-bold uppercase">{s.label}</p>
                  <h3 className="text-2xl font-bold mt-1">{s.count}</h3>
                </div>
                <div className="p-2.5 rounded-lg" style={{ background: s.color + '18' }}>
              ...
              <tbody className="divide-y divide-gray-50">
                {currentItems.length === 0 ? (
                  ...
                  <tr key={ord.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-4 font-bold text-[#E8650A]">{ord.order_number || '—'}</td>
                    ...
                    <td className="px-5 py-4">
                      ...
                      <p className="text-xs text-gray-400">{ord.customer_phone} · {ord.delivery_city}</p>
                    </td>
                    <td className="px-5 py-4 max-w-[200px] truncate text-gray-600" title={getItemNames(ord.items)}>
                    ...
                    <td className="px-5 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${STATUS_BADGE[ord.status] || 'bg-gray-100 text-gray-800 border-gray-200'}`}>
                        {ord.status}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center gap-1 ml-auto"
                      >
          ...
          <div className="p-5 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)}
                className="px-4 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-bold disabled:opacity-40 cursor-pointer hover:bg-gray-50">
                {isHindi ? 'पिछला' : 'Prev'}
              </button>
