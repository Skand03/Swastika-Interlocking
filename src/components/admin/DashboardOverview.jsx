...
export default function DashboardOverview({ orders, customers, products, metrics, language }) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      {/* Welcome Banner */}
      <div
        className="rounded-2xl p-5 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 overflow-hidden relative"
        style={{
      ...
      >
        <div className="relative z-10">
          <p className="text-green-300/80 text-xs sm:text-sm font-bold uppercase tracking-widest mb-2">Admin Portal</p>
        ...
        <div className="relative z-10 opacity-20 text-white hidden sm:block" style={{fontSize: '100px', lineHeight: 1, fontFamily: 'Material Symbols Outlined', fontVariationSettings: "'FILL' 1"}}>
          bar_chart
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {[
          ...
          <div key={card.label} className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition-shadow">
            <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center" style={{ background: card.bg }}>
              <span className="material-symbols-outlined" style={{ color: card.color, fontSize: '22px' }}>{card.icon}</span>
            </div>
            <div>
              <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wide text-gray-400">{card.label}</p>
              <p className="text-xl sm:text-2xl font-extrabold text-gray-800">{card.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-4 sm:p-6 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              ...
              <thead className="bg-gray-50">
                <tr>
                  {['Order ID', 'Customer', 'Product', 'Status', 'Date'].map(h => (
                    <th key={h} className="px-4 sm:px-5 py-3 font-bold text-xs text-gray-500 uppercase">{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {recentOrders.length === 0 ? (
                  ...
                  return (
                    <tr key={ord.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 sm:px-5 py-3 font-bold text-[#E8650A]">#SW-{ord.id}</td>
                      <td className="px-4 sm:px-5 py-3 font-semibold text-gray-700">{ord.customer_name}</td>
                      <td className="px-4 sm:px-5 py-3 text-gray-500 truncate max-w-[150px] sm:max-w-[200px]">
                        {(() => {
                          try {
                      ...
                      <td className="px-4 sm:px-5 py-3">
                        <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] font-bold" style={{ background: sc.bg, color: sc.text }}>{ord.status}</span>
                      </td>
                      <td className="px-4 sm:px-5 py-3 text-xs text-gray-400">{ord.created_at?.split(' ')[0]}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {/* Quick Stats Panel */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6 space-y-4 md:space-y-5">
          ...
          {[
            ...
            <div key={s.label} className="flex items-center gap-4 p-4 rounded-xl" style={{ background: s.bg }}>
              ...
              <div className="flex-1">
                ...
                <p className="text-xl sm:text-2xl font-extrabold" style={{ color: s.color }}>{s.count}</p>
              </div>
            </div>
          ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
          {[
            ...
            <div
              key={a.label}
              className="flex flex-col items-center gap-3 p-4 sm:p-5 rounded-xl border border-gray-100 cursor-pointer hover:shadow-md transition-all hover:-translate-y-1 active:scale-95"
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center" style={{ background: a.color + '18' }}>
