...
export default function CustomersDirectory({ language, customers, orders, selectedCustomer, setSelectedCustomer, customerSearch, setCustomerSearch }) {
  ...
  return (
    <section className="space-y-8 animate-fade-in" id="customers-directory">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h3 className="font-headline-md text-headline-md text-on-background text-xl font-bold">
            {isHindi ? 'ग्राहक निर्देशिका / Customer Directory' : 'Customer Directory / ग्राहक निर्देशिका'}
          </h3>
          <p className="text-on-surface-variant text-sm mt-1">
            {isHindi ? 'पंजीकृत ग्राहकों की सूची, उनके आदेश इतिहास और वित्तीय प्रदर्शन को देखें।' : 'View registered clients, their order history, and financial performance.'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-surface-container-highest flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg bg-[#E8650A]/10 flex items-center justify-center text-[#E8650A] flex-shrink-0">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-surface-container-highest flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg bg-[#1C2B1A]/10 flex items-center justify-center text-[#1C2B1A] flex-shrink-0">
            <span className="material-symbols-outlined">pending_actions</span>
          </div>
          <div>
            <p className="text-xs text-on-surface-variant font-semibold">{isHindi ? 'लंबित पूछताछ' : 'Open Inquiries'}</p>
            <h4 className="text-2xl font-bold">{openInquiriesCount}</h4>
          </div>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-surface-container-highest flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg bg-green-100 flex items-center justify-center text-green-700 flex-shrink-0">
            <span className="material-symbols-outlined">payments</span>
          </div>
          <div>
            <p className="text-xs text-on-surface-variant font-semibold">{isHindi ? 'कुल अनुमानित राजस्व' : 'Total Est. Revenue'}</p>
            <h4 className="text-2xl font-bold">₹{(totalSpentAll / 100000).toFixed(2)}L</h4>
          </div>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-surface-container-highest flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg bg-blue-100 flex items-center justify-center text-blue-700 flex-shrink-0">
            <span className="material-symbols-outlined">local_shipping</span>
          </div>
          <div>
            <p className="text-xs text-on-surface-variant font-semibold">{isHindi ? 'सक्रिय प्रोजेक्ट्स' : 'Active Projects'}</p>
            <h4 className="text-2xl font-bold">{activeJobs}</h4>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8 items-start">
            ...
            <table className="w-full text-left">
              <thead className="bg-surface-container-low border-b border-outline-variant/20">
                <tr className="text-on-surface-variant text-xs uppercase font-bold tracking-wider">
                  ...
                  <th className="px-6 py-4">{isHindi ? 'नाम / Name' : 'Name / नाम'}</th>
                  <th className="px-6 py-4">{isHindi ? 'शहर / City' : 'City / शहर'}</th>
                  <th className="px-6 py-4">{isHindi ? 'कुल ऑर्डर' : 'Total Orders'}</th>
                  <th className="px-6 py-4">{isHindi ? 'कुल खर्च' : 'Total Spent'}</th>
                  <th className="px-6 py-4">{isHindi ? 'स्थिति / Status' : 'Status / स्थिति'}</th>
                  <th className="px-6 py-4">{isHindi ? 'कार्रवाई' : 'Action'}</th>
                </tr>
              </thead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="font-bold break-all max-w-[200px]">{cust.full_name}</div>
                          <div className="text-xs text-on-surface-variant font-medium break-all max-w-[200px]">{cust.phone}</div>
                        </td>
                        <td className="px-6 py-4 font-semibold text-on-surface-variant break-all max-w-[150px]">{cust.city}</td>
                        <td className="px-6 py-4 font-bold">{custOrders.length}</td>
                        <td className="px-6 py-4 font-extrabold text-[#E8650A]">
        ...
        <div className="xl:col-span-1">
          {selectedCustomer ? (
            <div className="bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 space-y-6 flex flex-col items-center text-center sticky top-20">
              <div className="relative">
                <div className="h-20 w-20 rounded-full border-4 border-[#E8650A]/10 bg-[#E8650A]/5 text-[#E8650A] flex items-center justify-center font-bold text-2xl select-none">
              ...
              <div>
                <h4 className="font-bold text-lg text-on-surface break-all">{selectedCustomer.full_name}</h4>
                <p className="text-on-surface-variant text-xs mt-0.5 break-all">Verified Customer • {selectedCustomer.city}</p>
              </div>
              ...
              <div className="w-full grid grid-cols-2 gap-4">
                ...
                <div className="bg-surface-container-low p-3 rounded-xl border border-outline-variant/5">
                  ...
                  <p className="font-extrabold text-xs text-on-surface">
                    {selectedCustomer.created_at ? selectedCustomer.created_at.split(' ')[0] : 'N/A'}
                  </p>
                </div>
              </div>

              <div className="w-full text-left space-y-3 pt-3 border-t border-outline-variant/10 text-xs">
                <p className="text-on-surface break-all"><strong className="font-bold text-on-surface-variant">Phone:</strong> +91 {selectedCustomer.phone}</p>
                <p className="text-on-surface break-all"><strong className="font-bold text-on-surface-variant">City:</strong> {selectedCustomer.city}</p>
              ...
              <a 
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full py-3 bg-[#25D366] text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 transition-all text-sm shadow-md"
              >
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-8 text-center text-on-surface-variant text-sm font-bold">
